...
async function main() {
  console.log('Seeding database for June 2026...');
  ...
  for (let i = 0; i < houses.length; i++) {
    balanceData.push({
      houseId: houses[i].id,
      previousBalance: 100 + Math.floor(seededRandom(houses[i].id, 0, 99) * 2000),
      currentBalance: 0,
      dairyId: dairy.id,
    });
  }
  ...
  for (let day = 1; day <= 30; day++) {
    for (const house of houses) {
      ...
      const r1 = seededRandom(house.id, day, 1);
      const r2 = seededRandom(house.id, day, 2);
      const r3 = seededRandom(house.id, day, 3);
      ...
      const opening = 10 + Math.floor(seededRandom(house.id, day, 4) * 50);

      const deliveryDate = new Date(2026, 5, day);
  ...
  console.log(`  ✓ Delivery logs created (June 1-30: ${logCount} logs)`);
