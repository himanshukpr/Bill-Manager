...
import { IndianRupee, Plus, Search, Receipt, History, Check, ChevronDown, Rows3, ChevronLeft, ChevronRight, Edit2, Trash2, Eye, Calendar, MessageCircle } from 'lucide-react'
...
import { getSessionAuth, apiValidatePassword, getDairyIdFromCookie } from '@/lib/auth'
...
export default function ReceiptsPage() {
  ...
  }, [customBillOpen, customBillFromDate, customBillToDate, summaryHouse, dairyId])
  ...
  const matchingBills = useMemo(() => {
    // Use bill.month/year (1-indexed) to match summaryPeriod (0-indexed month) — same as houses page
  ...
  useEffect(() => {
    const fetchPeriodData = async () => {
      ...
      try {
        ...
        const allLogs = await deliveryLogsApi.list({ houseId: parseInt(formHouseId), dairyId: dairyId ?? undefined })
  ...
  async function openView(h: House) {
    try {
      ...
      setSummaryHouse(full)
    } catch {
      // keep current summaryHouse
    }
  }
  ...
  const handleExportSummaryPdf = useCallback(() => {
    ...
    if (summaryHouse.area) doc.text(`Area: ${summaryHouse.area}`, 14, 26)
                  ...
                  <div className="p-2 space-y-2 border-t border-border">
                    ...
                    {formFromDate && formToDate && (
                      <Button
                        ...
                        onClick={() => {
                          const house = houses.find(h => String(h.id) === formHouseId)
                          if (house) void openSummary(house)
                        }}
                      >
                        View Full Summary
                      </Button>
