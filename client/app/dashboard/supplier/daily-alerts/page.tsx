...
export default function SupplierDailyAlertsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const visibleConfigs: HouseConfig[] = configs.filter((config) => {
        if (config.shift === 'morning') return config.supplierId === auth.uuid
        return config.shift === 'evening'
      })

      const nextRows: AlertRow[] = visibleConfigs
        .map((config) => {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Add quick reminders for your route houses so you do not miss special instructions.
        </p>
      </div>
      ...
      <div className="relative">
        ...
        <Input
          placeholder="Search by house or alert text"
          value={search}
      ...
      <div className="overflow-hidden rounded-2xl border border-border bg-card">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
            ...
            <p className="font-medium">No route houses found</p>
            <p className="mt-1 text-sm">Ask admin to configure route houses first.</p>
          </div>
        ) : (
          <>
