...
import { apiListDairies, getSessionAuth, dashboardPath, type DairyInfo } from "@/lib/auth"
    ...
    <section className="rounded-3xl border border-border/70 bg-card/90 p-7 shadow-[0_30px_80px_-36px_rgba(15,23,42,0.22)] backdrop-blur-sm transition-colors duration-300 dark:border-white/10 dark:bg-slate-950/80 dark:shadow-[0_30px_80px_-36px_rgba(0,0,0,0.7)] sm:p-9">
            ...
            <div className="space-y-2">
              {filtered.map((d) => (
                <button
                  key={d.id}
                  type="button"
                  onClick={() => router.push(`/dairy/${d.id}/auth`)}
                  className="flex w-full items-center gap-4 rounded-xl border border-border bg-background/80 px-4 py-3 text-left transition-all duration-300 hover:border-primary/40 hover:bg-primary/[0.04] hover:shadow-sm focus:outline-none focus:ring-2 focus:ring-primary/15 dark:border-slate-700 dark:bg-slate-900 dark:hover:border-slate-500 dark:hover:bg-slate-800/80 dark:focus:ring-slate-700"
                >
      ...
      <div className="mt-6 border-t border-border pt-5 text-center">
        <p className="text-sm text-slate-600 dark:text-slate-400">
          Don&apos;t have a dairy?{" "}
          <Link href="/dairy/register" className="font-semibold text-slate-900 hover:underline dark:text-slate-100">
            Register New Dairy
          </Link>
        </p>
      </div>
